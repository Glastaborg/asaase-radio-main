...
if (isset($_GET['leave_id']) && isset($_GET['action'])) {
    ...
    if ($leave_data) {
        ...
        if (mysqli_query($dbcon, $update_query)) {
            echo "<script>alert('Leave request has been $leave_status.'); window.location.href='subleavereq.php';</script>";
        } else {
            $error_message = mysqli_error($dbcon);
            echo "<script>alert('Error: $error_message'); window.location.href='subleavereq.php';</script>";
        }
    } else {
        echo "<script>alert('Error: Invalid leave request.'); window.location.href='subleavereq.php';</script>";
    }
} else {
    echo "<script>alert('Error: Invalid parameters.'); window.location.href='subleavereq.php';</script>";
}
