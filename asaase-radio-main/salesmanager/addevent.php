...
if (isset($_POST['add_event'])) {
  $event_name = mysqli_real_escape_string($dbcon, $_POST['event_name']);
  $event_date = mysqli_real_escape_string($dbcon, $_POST['event_date']);
  $event_budget = mysqli_real_escape_string($dbcon, $_POST['event_budget']);
  $activity_brief = mysqli_real_escape_string($dbcon, $_POST['activity_brief']);
  $status = mysqli_real_escape_string($dbcon, $_POST['status']);
  $employee_id = $sessemp_id; // Replace with actual employee ID from your session or form

  $query = "INSERT INTO event (event_name, event_date, budget, description, employee_id, created_date) 
              VALUES ('$event_name', '$event_date', '$event_budget', '$activity_brief', $employee_id, NOW())";

  if (mysqli_query($dbcon, $query)) {
    $_SESSION['msg'] = "Event added successfully";
  } else {
    $_SESSION['msg'] = "Failed to add event: " . mysqli_error($dbcon);
  }
          ...
          <form class="columns is-multiline" action="addevent.php" method="post" enctype="multipart/form-data">
            ...
            <div class="column is-half">
              ...
              <div class="field">
                ...
                <div class="control">
                  <input class="input" id="event_date" name="event_date" min="<?php echo date("Y-m-d"); ?>" type="date" required>
                </div>
              </div>
              <div class="field">
                <label for="" class="label">Event Budget (GH₵)<span class="has-text-danger">*</span></label>
                <div class="control">
                  <input type="number" class="input" name="event_budget" required>
                </div>
              </div>
              <div class="field">
                <label for="" class="label">Activity Brief<span class="has-text-danger">*</span></label>
                <div class="control">
                  <input type="text" class="input" name="activity_brief" required>
                </div>
              </div>
              <div class="field">
