...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if ($employee_id && $reason && $date_start && $date_end) {
    ...
    $currentDate = strtotime($date_start);
    ...
    // Check if working days exceed the limit
    if ($workingDays > 30) {
      echo "Error: Leave period exceeds 30 working days.";
    } else {
      // Prepare the SQL statement
      $stmt = $dbcon->prepare("INSERT INTO employee_leave (employee_id, leave_status, applied_date, reason, date_from, date_to, archive_leave) VALUES (?, ?, ?, ?, ?, ?, ?)");
      $stmt->bind_param("sssssss", $employee_id, $leave_status, $applied_date, $reason, $date_start, $date_end, $archive_leave);
