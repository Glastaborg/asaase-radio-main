...
//Report on Agency
  if(isset($_POST['genagency'])){
    ...
    if(mysqli_affected_rows($dbcon) <= 0){
          ...
          echo '<div class="column is-full"><p class="has-text-centered"><img src="../images/asaase banner.webp" alt="banner logo" width="300" height="120"><p>';
          echo '<h1 class="title is-size-5 has-text-centered pb-3">Report on '.$sessbranch.' Agencies</h1>';
          echo '<div class=" py-5"><table class="table is-fullwidth is-striped is-narrow is-hoverable">';
          echo '<thead>
                  <tr>
                    <th class="is-size-6 ">Agency Name</th>
                    <th class="is-size-6 ">Agency Email</th>
                    <th class="is-size-6 ">Agency Phone Number</th>
                    <th class="is-size-6 ">Agency Address</th>
                  </tr>
                </thead>
                <tbody>';
          while ($row = mysqli_fetch_array($result)) {
            echo '<tr>
                <td class="is-size-6">'.$row['agency_name'].'</td>
                <td class="is-size-6">'.$row['agency_email'].'</td>
                <td class="is-size-6">'.$row['agency_phone'].'</td>
                <td class="is-size-6">'.$row['agency_location'].'</td></tr>';
              }
          echo '</tbody></table></div></div>';
    }
  }
