        ...
        <div class="columns is-multiline has-background-white mx-1 mb-3 p-3">
          ...
          <div class="column is-full">
            <div class="columns">
              ...
              <form method="POST" action="" class="column is-half">
                  ...
                  <tr class="datatr" >
                    ...
                    <td class="is-size-6 is-size-7-mobile has-text-right pr-6">
                     ...
                     <a class="button is-size-7 is-info m-1" title="update"  href="part-a.php?perf_part_a_id=<?php echo $row['perf_id']; ?>">
                      ...
                      <a class="button is-size-7 is-danger m-1" title="delete"  href="perf-hr.php?perf_id=<?php echo $row['perf_id']; ?>">
          ...
          <?php 
                //for other employees
                $emp_query = "SELECT * FROM perf_rev
                INNER JOIN employee ON employee.employee_id = perf_rev.perf_emp_id
                INNER JOIN perf_part_b ON perf_part_b.perf_id = perf_rev.perf_id
                WHERE perf_rev.perf_emp_id <> '$sessemp_id'
                ORDER BY perf_date DESC";
                $emp_result = mysqli_query($dbcon, $emp_query);  
          ?>
            ...
            <div class="table-container">
              <table class="table is-fullwidth is-striped is-narrow is-hoverable" id="table">
                ...
                <?php while($emp_row = mysqli_fetch_array ($emp_result)){ ?>
                <tbody>
                  <tr class="datatr" >
                    <td class="is-size-6 is-size-7-mobile"><?php echo $emp_row['employee_id']; ?></td>
                    <td class="is-size-6 is-size-7-mobile"><?php echo $emp_row['department']; ?></td>
                    <td class="is-size-6 is-size-7-mobile"><?php echo $emp_row['perf_date']; ?></td>
                    <td class="is-size-6 is-size-7-mobile"><?php echo $emp_row['perf_period']; ?></td>
                    <td class="is-size-6 is-size-7-mobile has-text-right pr-6">
                      <?php //if ($row['leave_status'] !== 'Granted' AND $row['leave_status'] !== 'Rejected') {

                     ?>
                     <a class="button is-size-7 is-info m-1" title="update"  href="perf-hr-preview.php?perf_id=<?php echo $emp_row['perf_id']; ?>">
                        <i class="fas fa-edit"></i>
                      </a>
