                  ...
                  <table class="table is-narrow is-fullwidth has-background-transparent">
                    <tbody>
                      ...
                      $result = mysqli_query($dbcon, $query);
                        ...
                        while($row = mysqli_fetch_array ($result)){
                      ?>
                      <tr class="datatr" onclick="window.location='viewemployees.php?employee_id=<?php echo $row['employee_id']; ?>';" style="cursor:pointer;" onMouseOver="this.style.background='#c4c4c4'" onMouseOut="this.style.background='#ffffff'"><td class="is-size-6 ">
                          <figure class="image is-48x48">
                              <?php
                                $imagequery = "SELECT * FROM employee_image WHERE employee_id = '".$row['employee_id']."'";
                                $imageresult = mysqli_query($dbcon, $imagequery);
