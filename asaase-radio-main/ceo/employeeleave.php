...
$query = "SELECT *, DATEDIFF(date_to, date_from) AS date_diff 
FROM employee
INNER JOIN employee_leave ON employee.employee_id = employee_leave.employee_id
WHERE archive_leave = 'No' 
AND employee.deleted = '0' 
AND (employee.job_description = 'Department Head' 
     OR employee.job_description = 'General Manager' 
     OR employee.job_description = 'Branch Manager'
     OR employee.job_description = 'Executive Secretary'
     OR employee.job_description = 'Managing News Editor'
     OR employee.job_description = 'Programs Manager')
...
// Function to calculate the number of working days
function calculate_days_left($date_from, $date_to, $holidays)
{
                        ...
                        <table class="table is-fullwidth is-striped is-narrow is-hoverable" id="table">
                            <thead>
                                <tr>
                                    ...
                                    <th class="is-size-6 is-size-7-mobile">Days Remaining</th>
                            ...
                            <tbody>
                                ...
                                while ($row = mysqli_fetch_array($result)) {
                                    ...
                                    $days_left = calculate_days_left($row['date_from'], $row['date_to'], $holidays);
                                ?>
                                    <tr>
                                        ...
                                        <td class="is-size-6 is-size-7-mobile"><?php echo $row['days_requested']; ?></td>
                                        <td class="is-size-6 is-size-7-mobile"><?php echo $row['remaining_leave_days']; ?></td> <!-- Display days left -->
                                        <td class="is-size-6 is-size-7-mobile">
                                            <?php if ($row['leave_status'] === 'Pending') { ?>
                                                <!-- Approve Leave -->
                                                <a class="button is-size-7 is-success m-1" title="approve leave" href="process_leave_request.php?leave_id=<?php echo $row['leave_id']; ?>&action=approve">
                                                    <i class="fas fa-check"></i> Approve
                                                </a>
                                                <!-- Reject Leave -->
                                                <a class="button is-size-7 is-danger m-1" title="reject leave" href="process_leave_request.php?leave_id=<?php echo $row['leave_id']; ?>&action=reject">
