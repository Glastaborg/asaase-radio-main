...
if (isset($_POST['create'])) {
  ...
  //perf_period = '$perf_period' AND add this to where
  
  //delete next time
  $perf_period = "Testing Quater Quarter - ".$year;

  if ($perf_period) {
    $check_entry = "SELECT perf_period FROM perf_rev WHERE  perf_emp_id = '$sessemp_id'";
        ...
        <div class="columns is-multiline has-background-white mx-1 mb-3 p-3">
          <div class="column is-full">
            <div class="columns is-align-items-center py-1">
              <div class="column is-two-thirds">
                <h1 class="title is-size-5">Performace Review</h1>
              </div>
              <div class="column">
                <div class="field">
          ...
          <div class="column is-full">
            <div class="columns">
              ...
              <form method="POST" action="perf-review.php" class="column is-half">
                <input type="text" name="perf_emp_id" value="<?php echo $sessemp_id; ?>" hidden>
                <input type="date" name="perf_date" value="<?php echo date('Y-m-d'); ?>" hidden>
                <p class="has-text-right">
                    <button class="button is-info" name="create">Create Review</button>
                </p>
              </form>
              <?php } ?>
            </div>
          ...
          <div class="column is-full is-align-items-center">
            <h4 class="subtitle">My Performance Review</h4>
                ...
                <tbody>
                  <tr class="datatr" >
                    ...
                    <td class="is-size-6 is-size-7-mobile has-text-right pr-6">
                     ...
                     ?>
                     <a class="button is-size-7 is-info m-1" title="update"  href="part-a.php?perf_part_a_id=<?php echo $row['perf_id']; ?>">
                        <i class="fas fa-edit"></i>
                      </a>
                      <a class="button is-size-7 is-danger m-1" title="delete"  href="perf-review.php?perf_id=<?php echo $row['perf_id']; ?>">
                  ...
                  <?php } } else {
                    echo '<tr><td><p class="subtitle is-size-6 py-3 has-text-danger">You have no personal performance review data</p></td></tr>';
                  }?>
                </tbody>
              </table>
            </div>
          ...
          <?php 
                //for supervisor only
                if ($sessjob_desc === "Supervisor") {
                $sup_query = "SELECT * FROM perf_rev
                INNER JOIN employee ON employee.employee_id = perf_rev.perf_emp_id
                INNER JOIN perf_part_b ON perf_part_b.perf_id = perf_rev.perf_id
                WHERE employee.branch = '$sessbranch' AND employee.department = '$sessdepartment'
                AND employee.employee_id <> '$sessemp_id'
                ORDER BY perf_date DESC";
                $sup_result = mysqli_query($dbcon, $sup_query);  
          ?>
            <div class="column is-full is-align-items-center mt-6">
            <h4 class="subtitle">Department Appraisee's Performance Review</h4>
            <div class="table-container">
              <table class="table is-fullwidth is-striped is-narrow is-hoverable" id="table">
                
                <?php if (mysqli_num_rows($sup_result) > 0) { ?>
                <thead>
                  <tr >
                ...
                <?php while($sup_row = mysqli_fetch_array ($sup_result)){ ?>
                <tbody>
                  <tr class="datatr" >
                    <td class="is-size-6 is-size-7-mobile"><?php echo $sup_row['employee_id']; ?></td>
                    <td class="is-size-6 is-size-7-mobile"><?php echo $sup_row['perf_date']; ?></td>
                    <td class="is-size-6 is-size-7-mobile"><?php echo $sup_row['perf_period']; ?></td>
                    <td class="is-size-6 is-size-7-mobile has-text-right pr-6">
                      <?php //if ($row['leave_status'] !== 'Granted' AND $row['leave_status'] !== 'Rejected') {

                     ?>
                     <a class="button is-size-7 is-info m-1" title="update"  href="part-a.php?perf_part_a_id=<?php echo $sup_row['perf_id']; ?>">
                        <i class="fas fa-edit"></i>
                      </a>
                  ...
                  <?php } } else {
                    echo '<tr><td><p class="subtitle is-size-6 py-3 has-text-danger">There are no performance review data</p></td></tr>';
                  }?>
                </tbody>
              </table>
            </div>
