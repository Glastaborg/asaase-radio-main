...
//Adding Employee Backend
  if (isset($_POST['addemployee'])) {
   ...
   if ($_POST['branch']  === "Accra") {
    ...
    // branch employee id
    $query = "INSERT INTO `employee` (employee_id, firstname, lastname, email, phone, dob, password, position, job_description, department, branch, prev_name, pref_name, archive_emp, employee_type, date_joined, unit)
              VALUES ('$employee_id', '$firstname', '$lastname', '$email', '$phone', '$dob', '$password', '$position', '$job_description', '$department', '$branch', '$prev_name', '$pref_name', '$archive_emp', '$employee_type', '$date_joined' , '$unit')";
    $result = mysqli_query($dbcon, $query);
    if($result){
       include"includes/successmsg.php";
    }else{
