              ...
              <form class="columns is-multiline" action="progupdate.php?prog_upt_id=<?php echo $_GET['prog_upt_id']; ?>" method="post" enctype="multipart/form-data">
                <div class="column is-half">
                  ...
                  <div class="field">
                    ...
                    <div class="control">
                      <textarea class="textarea" name="program_update" rows="3" placeholder="Enter Asset Description" required readonly> <?php echo $data['program_update']; ?></textarea>
                    </div>
                  </div>
                  <div class="field">
                    <label class="label">Date <span class="has-text-danger">*</span></label>
                    <div class="control">
                      <input class="input" name="update_date" type="date" value="<?php echo $data['update_date']; ?>" required readonly>
                    </div>
                  </div>
                  <div class="field">
                    <label class="label">Comments <span class="has-text-danger">*</span></label>
                    <div class="control">
                      <textarea class="textarea" name="prog_comment" rows="3" <?php if(empty($data['prog_comment'])){echo 'placeholder="Enter Comments"';} ?> required><?php if(!empty($data['prog_comment'])){echo $data['prog_comment'];} ?> </textarea>
                    </div>
                  </div>
                  <div class="field is-grouped my-5">
                    <p class="control is-expanded is-size-6"></p>
                    <p class="control">
                      <button class="button is-info" name="progcomment">
                        &nbsp; Add Comment &nbsp;
                      </button>
                    </p>
                  </div>
